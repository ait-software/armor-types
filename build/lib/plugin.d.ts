...
import { ArmorLogger } from './logger';
...
export interface PluginStatic<P extends Plugin> {
    /**
     * Allows a plugin to modify the Armor server instance.
     */
    updateServer?: UpdateServerCallback;
    /**
     * Plugins can define new methods for the Armor server to map to command names, of the same
     * format as used in Armor's `routes.js`, for example, this would be a valid `newMethodMap`:
     * @example
     * {
...
export interface Plugin {
    ...
    /**
     * Handle an Armor command, optionally running and using or throwing away the value of the
     * original Armor behavior (or the behavior of the next plugin in a plugin chain).
     */
